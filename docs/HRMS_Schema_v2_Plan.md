...
| 5. Frontend | 2026-05-12 | 2026-05-13 | d49c2ca + 8a4057a + f1c15ba | ✅ Done — Phase 5a (`lib/status/maps.ts` INT→label maps + StatusBadge `entity`+`code` mode); Phase 5b (`lib/api/*` + `lib/hooks/*` + `lib/api/masters.ts` + layout/auth context rewritten for v2 contracts); Phase 5c (121 files: all pages across admin/manager/employee/payroll roles, per-entity badge components, feature components, dashboards, masters CRUD). `pnpm --filter @nexora/web typecheck` exits 0; `lint` clean (4 warnings); `build` clean; smoke test on all 4 demo accounts returns correct INT user payloads (admin roleId=4, manager=2, employee=1, payroll=3); every role's `/<role>/dashboard` + key list pages render 200 against a real backend. |
| 6. .sql export | 2026-05-13 | 2026-05-13 | (this commit) | ✅ Done — `apps/api/prisma/schema_v2.sql` (~100 KB) generated via `mysqldump --single-transaction --no-tablespaces --skip-add-locks --skip-comments --default-character-set=utf8mb4 --databases nexora_hrms`. Self-contained (CREATE DATABASE + USE inside the dump). 35 tables, 0 FK constraints, all 26 INT-code COLUMN_COMMENTs preserved. Restored cleanly into a throwaway DB and counts matched the source. Top of file has a banner explaining how devops applies it, what data is inside, demo account credentials, and the regenerate command. |
| 7. QA | 2026-05-13 | 2026-05-13 | 1cf4bae + (this commit) | 🟡 Yellow — full QA report at `docs/HRMS_v2_QA_Report.md`. 1 blocker fixed inline (`lastSeq` → `number` in 4 raw-SQL code generators, commit `1cf4bae`). 8 modules verified green (auth, leave, attendance, regularisation, payroll happy-path, notifications, role gates, BL-021/029/034/041/044/047 enforced). 5 HIGH-severity defects open: D1 masters endpoints not registered (404), D2 performance cycle status returned as string, D3 goal outcome returned as string, D4 audit log uses `module: string` instead of `moduleId: number` + `moduleName`, D8 audit `moduleId` query filter ignored. Plus 1 MEDIUM (D6 payroll status leak via `mapRunStatusToString`) and 2 LOW. ~2 hours of follow-up fixes needed for GREEN. |
