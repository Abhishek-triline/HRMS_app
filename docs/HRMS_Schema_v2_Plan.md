...
| 7. QA | 2026-05-13 | 2026-05-13 | 1cf4bae + fe4487b + (this commit) | 🟢 Green — full report at `docs/HRMS_v2_QA_Report.md`. Initial pass found 9 defects (1 BLOCKER, 5 HIGH, 1 MEDIUM, 2 LOW); 8 fixed in the same QA cycle. Blocker D0 fixed in `1cf4bae` (lastSeq→number in 4 raw-SQL counters). Initial report committed at `fe4487b`. This commit ships D1 (masters endpoints), D2 (perf cycle.status INT), D3 (goal.outcomeId INT), D4 (audit moduleId+moduleName), D5 (leave approve leaveTypeId), D8 (audit moduleId filter). D6 was a false alarm (mapRunStatusToString only used for PDF). Only D7 (LOW, operational note about dummy seed) remains. Final typecheck exit 0, 54/54 tests pass, all retests pass against live API. |
